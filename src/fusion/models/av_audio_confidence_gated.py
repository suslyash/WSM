"""Audio-confidence-gated zero-initialized residual fusion candidate C."""
...
class WSMAudioConfidenceGatedModel(BaseModel):
    def __init__(self, audio_checkpoint_path: str, audio_feature_dim: int = 768,
        ...
        if (audio_feature_dim, audio_hidden_dim, video_feature_dim, num_tasks) != (768, 192, 512, 2):
            raise ValueError("audio-confidence candidate C requires audio=768/192, video=512, tasks=2")
        ...
        self.gate_heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(audio_hidden_dim + 1, gate_hidden_dim), nn.GELU(),
    ...
    def forward(self, batch: Any) -> ModelOutput:
        ...
        corrections, gates = [], []
        for task, (gate, residual) in enumerate(zip(self.gate_heads, self.residual_heads)):
            confidence = audio_out["base_logits"][:, task:task + 1].abs()
            gate_value = torch.sigmoid(gate(torch.cat((
                audio_out["task_features"][:, task], confidence), dim=1
            )))
            correction = residual(torch.cat((
                audio_out["task_features"][:, task], video_features
            ), dim=1))
            gates.append(gate_value)
            corrections.append(correction)
        gate_values = torch.cat(gates, 1)
        residual_logits = torch.cat(corrections, 1)
        residual_logits = residual_logits * gate_values * available[:, 1:2].to(residual_logits.dtype)
...
@MODELS.register("wsm_av_audio_confidence_gated_model")
def wsm_av_audio_confidence_gated_model(context: Any | None = None, **params: Any) -> WSMAudioConfidenceGatedModel:
    params = dict(params)
    ...
    return WSMAudioConfidenceGatedModel(**params)
