#!/usr/bin/env python3
"""Extract DEPART-style frozen CLIP temporal body-ROI features."""
    ...
    try:
        ...
        if not args.raw_frame_debug:
            if args.yolo_weights is None:
                raise ValueError("--yolo-weights is required for normal V1 ROI extraction")
            if not args.yolo_weights.is_file():
                raise FileNotFoundError(f"YOLOv8 weights file is missing: {args.yolo_weights}")
        ...
        extractor = ClipVideoFeatureExtractor(
            ...
            device=args.device, detector=detector, raw_frame_debug=args.raw_frame_debug,
        )
        successes, failures = [], []
        for row in candidates:
            ...
            fingerprint = build_cache_fingerprint(
                ...
                detector_weights_sha256=None if detector is None else detector.weights_sha256,
                detector_confidence=None if detector is None else detector.confidence,
                detector_iou=None if detector is None else detector.iou,
                detector_imgsz=None if detector is None else detector.imgsz,
            ...
            if artifact_path.exists() and not args.overwrite:
                failures.append({"segment_id": segment_id, "category": "cache_exists", "cache_path": str(artifact_path)})
                continue
            result = extractor.extract(source)
            if not result.success:
                failures.append({"segment_id": segment_id, "category": result.failure_kind or "extraction_failure",
                                 "reason": result.error, "source_path": str(source)})
                continue
            save_cache_artifact(artifact_path, segment_id=segment_id, source_path=str(source), result=result,
            ...
            successes.append({"segment_id": segment_id, "cache_path": str(artifact_path),
                              "cache_fingerprint": fingerprint, "detection_coverage": result.detection_coverage})
        coverage = [item["detection_coverage"] for item in successes]
        report = {
            "schema_version": "wsm-depart-video-cache-report-v1", "manifest_path": str(manifest_path),
            "manifest_fingerprint": manifest_fingerprint, "cache_root": str(args.cache_root),
            "cache_fingerprints": [item["cache_fingerprint"] for item in successes],
            ...
            "limit": args.limit, "video_available_candidates": len(candidates),
            "success_count": len(successes), "failure_count": len(failures),
            "no_body_detected_count": sum(item["category"] == "no_body_detected" for item in failures),
            "video_read_or_model_load_failure_count": sum(item["category"] == "video_read_or_model_load" for item in failures),
            "aggregate_detection_coverage": sum(coverage) / len(coverage) if coverage else 0.0,
            "successes": successes, "failures": failures,
            "test_metrics_inspected": False, "labels_passed_to_encoder": False,
        }
        args.report_output.parent.mkdir(parents=True, exist_ok=True)
        args.report_output.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print(json.dumps({"success_count": len(successes), "failure_count": len(failures),
                          "manifest_fingerprint": manifest_fingerprint}, sort_keys=True))
        return 0
